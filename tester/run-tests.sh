...
print_error_message () {
    ...
    echo "  what results produced by your program: $outputdir/$testnum.$filetype"
    echo "  compare the two using diff, cmp, or related tools to debug, e.g.:"
    echo "  prompt> diff $testdir/$testnum.$filetype $outputdir/$testnum.$filetype"
...
run_and_check () {
    ...
    run_test $testdir $testnum $verbose $outputdir
    rccheck=$(check_test $testdir $testnum $contrunning rc $outputdir)
    outcheck=$(check_test $testdir $testnum $contrunning out $outputdir)
    errcheck=$(check_test $testdir $testnum $contrunning err $outputdir)
    othercheck=0

    if [[ -f $testdir/$testnum.other ]]; then
	othercheck=$(check_test $testdir $testnum $contrunning other $outputdir)
    fi
	...
	if (( $rccheck == 1 )); then
	    print_error_message $testnum $contrunning rc $outputdir
	fi
	if (( $outcheck == 1 )); then
	    print_error_message $testnum $contrunning out $outputdir
	fi
	if (( $errcheck == 1 )); then
	    print_error_message $testnum $contrunning err $outputdir
	fi
	if (( $othercheck == 1 )); then
	    print_error_message $testnum $contrunning other $outputdir
	fi
    fi
}
...
#
verbose=0
testbasicdir="tests/basic"
outputbasicdir="tests-out/basic"
outputdir="tests-out"
testdir="tests"
contrunning=0
skippre=0
...
echo "Started Basic Tests"
# run all basic tests
run_and_check $testbasicdir 1 $contrunning $verbose 1 $outputbasicdir
run_and_check $testbasicdir 2 $contrunning $verbose 1 $outputbasicdir
run_and_check $testbasicdir 3 $contrunning $verbose 1 $outputbasicdir
run_and_check $testbasicdir 4 $contrunning $verbose 1 $outputbasicdir
echo "Finished Basic Tests"

# run all tests
(( testnum = 1 ))
while true; do
    run_and_check $testdir $testnum $contrunning $verbose 0 $outputdir
    (( testnum = $testnum + 1 ))
done
